...
export default function WeekOverview({ days }: { days: WeekRangeDays }) {
  ...
  function setRange(next: WeekRangeDays) {
    router.replace(`/week?range=${next}`);
  }
  ...
  async function fetchEventsForDate(date: string) {
    await fetchEvents(
      date,
      date,
      `Events on ${new Date(date + "T12:00:00").toLocaleDateString("en-US", {
        weekday: "short",
        month: "short",
        day: "numeric",
      })}`
  ...
  async function fetchEventsForCategory(colorId: string, colorName: string) {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Failed to fetch events");
      const data = await res.json();
      const filtered = data.events.filter((e: Event) => e.color_id === colorId);
      setEvents(filtered);
    } catch (err) {
      console.error("Error fetching events:", err);
      setEvents([]);
          ...
          {pieData.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                ...
                <Tooltip
                  formatter={(value) => [formatHours(Number(value)), "Time"]}
                />
              </PieChart>
            </ResponsiveContainer>
          ) : (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={dailyData}>
              ...
              />
              <Tooltip formatter={(value) => [`${value}h`, "Scheduled"]} />
              <Bar
                dataKey="hours"
                fill="#3b82f6"
                radius={[4, 4, 0, 0]}
